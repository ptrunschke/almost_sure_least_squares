...
if __name__ == "__main__":
    ...
    for rank in [4]:
        ...
        for it in range(1, max_iteration + 1):
            ...
            if False:
                ...
                update_core = least_squares(full_sample, full_weights, full_gradient, core_basis_l2)
                tt.core -= step_size * update_core.reshape(tt.core.shape)
                test_error = compute_test_error(tt)
                print(f"  Relative test set error: {test_error:.2e}")
            else:
                update_core = np.zeros(tt.core.size, dtype=float)
            ...
            if True:
                ...
                # debiasing_sample_size = 4 * core_basis_l2.dimension
                # debiasing_sample_size = int(10 * debiasing_sample_size * np.log(debiasing_sample_size))
                # debiasing_sample_size = core_basis_l2.dimension
                debiasing_sample_size = 10
                l_min = 0
                while l_min < 0.5:
                    ...
                    debiasing_weights = 1 / core_basis_l2.core_space.christoffel(debiasing_sample, core_basis_l2.bases)
                    B = core_basis_l2(debiasing_sample)
                    G = B * debiasing_weights @ B.T / debiasing_sample_size
                    l_min = np.linalg.norm(G, ord=-2)
                    l_max = np.linalg.norm(G, ord=2)
                    print(f"  L2-Gramian spectrum: [{l_min:.2e}, {l_max:.2e}]")
                    break
                ...
                # debiasing_sample, debiasing_weights, debiasing_values = full_sample, full_weights, full_values
                # assert space in ["h1", "h10"]
                ...
                projected_debiasing_gradient = update_core @ core_basis_l2(debiasing_sample)
                assert np.allclose(projected_debiasing_gradient, 0)
                if True:
                    ...
                    norm_factor = debiasing_gradient**2 @ debiasing_weights / debiasing_sample_size
                    adv = norm_factor
                    adv *= 2 * core_basis_l2.dimension / debiasing_sample_size
                    print(f"  Additive debiasing variance: {adv:.2e} (norm factor: {norm_factor:.2e})")
                else:
                    step_size = 1
                    debiasing_core = least_squares(
                        debiasing_sample, debiasing_weights, debiasing_gradient - projected_debiasing_gradient, core_basis_l2
                    )
            ...
            sample_sizes.append((sample_sizes[-1] if sample_sizes else 0) + len(debiasing_sample))
    ...
    plot_path = plot_directory / f"{plot_file}_bkp.png"
